...
g1 <- player_df %>% 
  ...
  ggplot(aes(x = rating_avg, y = avg_score)) +
  geom_point(aes(x=3280, y=8.56), size=12, shape=21, color = "black") + 
  geom_point(color = "grey", alpha = 0.5) +
  geom_point(data = player_df %>% filter(N_participations >= 10, title %in% c("IM")),
             mapping = aes(x = rating_avg, y = avg_score), color = "#E69F00", alpha = 0.6) + 
  geom_point(data = player_df %>% filter(N_participations >= 10, title %in% c("GM")),
             mapping = aes(x = rating_avg, y = avg_score), color = "#0072B2", alpha = 0.7) + 
...
#             mapping = aes(x = rating_avg, y = avg_score), color = "darkblue", alpha = 1, size = 2) + 
  ...
  labs(x="Average player rating", y="Average player score") +
  theme_light()
...
We can look at the total prize money earned throughout 2023 to find Hikaru (\$29,000) winning almost twice as much as Magnus (\$15,850). Nakamural also won the tournament twice as often as Carlsen (18 vs 9 wins), but this is mostly due to the fact that he participated almost twice as often (74 vs 39).
...
table1 <- player_df %>% arrange(desc(winning_chance)) %>% 
  slice_head(n=10) %>% 
  mutate(winning_pct = paste0(round(winning_chance*100, 1), "%")) %>% 
  select(name, N_participations, N_1st, winning_pct)

table1 %>%
  knitr::kable(caption = "Table 1: Winning chance (First places/Tournament participations)", digits = 0) %>%
  kableExtra::kable_styling(bootstrap_options = "striped", full_width = FALSE)
``` 

```{r winning chance 2, results = 'asis', eval=TRUE, echo=FALSE, cache = TRUE}
DT::datatable(table1)
``` 
...
player_df %>% arrange(desc(avg_prize)) %>% 
  slice_head(n=10) %>% 
  mutate(total_prize = dollar(total_prize),
         avg_prize = dollar(avg_prize)) %>% 
  select(name, avg_prize, N_participations, total_prize, rating_best) %>%
...
When we look at players that participated in 10 or more Titled Tuesday tournaments (to have a somewhat solid foundation for winning percentages). We see that Magnus Carlsen has the highest winning percentage of all players (80% with white pieces and 72.7% with black pieces).
...
Magnus only lost 6.3% of the games with white. A statistics that is far ahead of the competition.
...
Link to the chess-com video on youtube (https://www.youtube.com/live/CbJSYohWnoc?si=74LgbhWh0EpRpUCr&t=8598).
...
We can make a similar chart for Magnus Carlsen who participated in 39 tournaments, played 399 games against 239 different opponents.
