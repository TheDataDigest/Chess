...
top_20_win_white <- winning_percentages %>% 
...
kable(top_20_win_white)
```


```{r, include = FALSE}
#And with the black pieces we see that a cut-off of 30 games might be too low. 
top_20_win_black <- winning_percentages %>% 
...
kable(top_10_win_white2)
```

And this result for black:
```{r}
top_10_win_black2 <- winning_percentages %>% 
  filter(username != "howitzer14") %>% 
...
top_20_lose_white <- losing_percentages %>% 
  filter(games_w_White >= 30,
...
```{r, include=TRUE}
top_10_lose_white2 <- losing_percentages %>% 
  filter(username != "howitzer14") %>% 
  filter(games_w_White >= 50,
         result_color == "lose_White") %>% 
  arrange(lose_white_pct) %>% 
...
# Best woman
It is not easy to rate which women performed the best in 2024. We can sort them by different criteria and then judge how often one ranked higher than the others.

### Summary table
I looked into 10 different criteria and reported the rank of these top 3 players.
I think it is a tie between Gulrukhbegim Tokhirjonova (24yo) and Aleksandra Maltsevskaya (22yo). Anna Shukham (16yo) is the clear third place.
Tokhirjonova has a better median and average ranking compared to Maltsevskaya. However Maltsevskaya is ahead in 6 out of 10 criteria. One could argue to combine the average opponents ratings faced and defeated into one category. Same could be said about winning percentages with white and black. However Tokhirjonova is clearly ahead of Maltsevskaya when it comes to winning percentages.

The table below shows their individual values and the rank compared to all women players.
The average tournament score is filtered for at least 5 participations. The winning percentages are ranked for women that played at least 30 games. The opponent rating ranks are filtered for at least 100 matches and the opponent rating defeated ranks are filter for at least 50 wins.
The absolute ranking is not as important as the relative ranking and there it is really close between the top two.
...
The table below shows the price money they earned. The highest placed women earns \$100 in each tournament.
Aleksandra Maltsevskaya (\$1400), Gulrukhbegim Tokhirjonova (\$1100), Anna Shukhman (\$700), would be the top 3.
...
```
The interactive table below shows all 532 women that participated in the Title tuesday tournaments.
...
top_30_women_white <- winning_percentages %>% 
...
kable(top_30_women_white)
```

```{r, include = FALSE}
top_30_women_black <- winning_percentages %>% 
  filter(username %in% women_users) %>% 
  filter(games_w_Black >= 30,
         result_color == "win_Black") %>% 
  arrange(desc(win_black_pct)) %>% 
...
kable(top_30_women_black)
```

```{r , include = FALSE}
women_df <- player_df %>% filter(name %in% women) %>% 
  select(name, title, fed, best_Place, score, rating_best, N_participations) %>% 
  arrange(best_Place) %>% slice_head(n=20)

kable(women_df)
```

```{r, include = FALSE}
#average competition faced, based on at least 100 games

matches_df %>% filter(hero_name %in% women) %>% 
  group_by(username, hero_name) %>% 
  summarize(average_opponent_rating = mean(opponent_rating),
            median_opponent_rating = median(opponent_rating),
            N_games = n()) %>% filter(N_games >= 100) %>% arrange(desc(median_opponent_rating)) %>% View()
```

```{r, include=FALSE}
#average competition defeated
...
matches_df %>% filter(hero_name %in% women) %>% 
  ...
  summarize(average_opponent_rating = mean(opponent_rating),
            median_opponent_rating = median(opponent_rating),
            N_games = n()) %>% filter(N_games >= 50) %>% arrange(desc(average_opponent_rating)) %>% View()
```
